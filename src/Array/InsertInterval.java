 ...
 * Example 1:
 * Given [1,3],[6,9],
 * insert and merge with [2,5]===> [1,5],[6,9]
 * <p/>
 * Example 2:
 * Given [1,2],[3,5],[6,7],[8,10],[12,16]
 * insert and merge with [4,9] ===> [1,2],[3,10],[12,16]
 *
 * @author epttwxz
 */
public class InsertInterval {
    ...
    public List<Interval> insert(List<Interval> intervals, Interval newInterval) {
        ...
        for (int i = 0; i < intervals.size(); i++) {
            ...
            if (lastInter == null) {
                ...
                // newInter<< curInter, add newInter to the list, and exit the merge
                ...
                if (newInterval.end <= curEnd && newInterval.start >= curStart) {
                    merged_inter.addAll(intervals);
                    break;
                }
                // curInter is covered by newInter
                if (newInterval.start <= curStart && newInterval.end >= curEnd){
                    merged_inter.add(newInterval);
                }
                ...
                if(newInterval.start>= curStart){
                    if(newInterval.start <=curEnd){
                        merged_inter.add(new Interval(curStart, Math.max(curEnd, newInterval.end)));
                    }
                    else{
            ...
            // has intervals in the list
                ...
                if(right>=curInter.start && left>=curInter.end){
                    merged_inter.remove(merged_inter.size() - 1);
                    merged_inter.add(new Interval(Math.min(left,curStart), Math.max(right, curEnd)));
    ...
    public  void test1() throws Exception{
        ...
        ****/
        ...
        List<Interval> output = new InsertInterval().insert(intervals, newInterval);
